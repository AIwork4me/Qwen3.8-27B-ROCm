...
| [`rocm-7.14/dflash2-validation.md`](rocm-7.14/dflash2-validation.md) | DFlash2 validation receipts (v0.1.14, 2026-08-21): the unpatched-pin architecture refusal, the PR #52816 port record, the 262144 KV-infeasibility, the 131072 boot, greedy byte-identity | **Validated** — DFlash2 speculative decoding on vLLM @131072 (draft 3.6 GiB; first vLLM cell at the interactive floor) |
...
| [`matrix-714/matrix.json`](matrix-714/matrix.json) | The declared cell universe: 58 cells — 30 measured (20 v0.1.0/v0.1.1 priority + 8 v0.1.2 Vulkan×MTP/unified + 2 v0.1.14 DFlash2), 20 planned (time-boxed), 8 dropped (vLLM ctx-32768 tier not offered) | Complete for the session; guarded regeneration (`gen-matrix.py --check`) |
| [`matrix-714/stability/`](matrix-714/stability/) | Stability & pairing sessions (receipts-only, never corpus cells): the Vulkan variance series (s1–s6, trigger-hunt) and the v0.1.14 DFlash2 same-session pairing (`dflash-pairing-2026-08-21/` — the with-vs-without basis for the dflash corpus cells) | Receipts-only namespace; conclusions integrate via dated rulings, never direct matrix edits |
| [`matrix-714/cells/`](matrix-714/cells/) | The 30 raw measured cells — every boot line, stream record, anchor result, engine args verbatim | 30/30 measured; 5 GGUF-hip cells recorded `measured(degraded)` (the greedy-degradation pit — not reproducing on Vulkan) |
